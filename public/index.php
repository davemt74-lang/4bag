...
<title>FourBag League Network</title>
<link rel="stylesheet" href="assets/app.css">
</head>
<body>
<header class="topbar"><div><strong class="brand">FOURBAG</strong><span class="tag">League Network</span></div><nav><a href="#leagues">Leagues</a><a href="#register">Register</a><a href="#product">Buy FourBag</a><a href="player.php">Player Account</a></nav></header>
<main>
<section class="hero"><div><span class="eyebrow">BACKYARD GAME • LOCAL LEAGUES • FOURBAG LIVE</span><h1>Play local.<br>Take the game home.</h1><p>Find an official 8-week FourBag league, register solo or with friends, and follow standings across the network.</p><div class="actions"><a class="btn primary" href="#leagues">Find a League</a><a class="btn" href="#product">Buy FourBag — $199</a><a class="btn" href="player.php">My FourBag History</a></div></div><div class="hero-card"><div class="big">32</div><span>players per venue</span><div class="big">8</div><span>weeks per season</span><div class="big">4</div><span>official boards</span></div></section>
...
async function api(action,options={}){const res=await fetch(`api.php?action=${encodeURIComponent(action)}`,{credentials:'same-origin',...options}),body=await res.json();if(!body.ok)throw new Error(body.error||'Request failed');return body.data}
...
document.getElementById('registerForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    const profileNote=data.profile_link?.linked?' Your Player Account history is linked.':'';
    if(data.payment_status==='awaiting_board_payment'){
      result.textContent=`FourBag order #${data.board_order_id} created.${profileNote} Opening secure checkout...`;
      try{
      ...
      result.textContent=`Your paid FourBag purchase includes this league registration.${profileNote}`;
    }else if(data.payment_status==='paid'||data.payment_status==='waived'){
      result.textContent=`Your league registration is confirmed.${profileNote}`;
    }else{
      result.textContent=`Registration created. The league registration payment is still pending.${profileNote}`;
    }
