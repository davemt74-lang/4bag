...
<body>
<header class="topbar"><div><strong class="brand">FOURBAG</strong><span class="tag">League Network</span></div><nav><a href="#leagues">Leagues</a><a href="#register">Register</a><a href="#product">Buy FourBag</a></nav></header>
<main>
...
async function loadSeasons(){try{const seasons=await api('seasons');seasonSelect.innerHTML=seasons.map(s=>`<option value="${s.id}">${s.venue_name} — ${s.name}</option>`).join('');leagueGrid.innerHTML=seasons.length?seasons.map(s=>`<article class="card"><span class="pill">${s.status.replaceAll('_',' ')}</span><h3>${s.venue_name}</h3><p>${s.name}</p><div class="meta">${s.city||''}${s.state?', '+s.state:''} • ${s.weeks} weeks</div><div class="fill"><strong>${s.registered_players}/${s.team_limit*s.players_per_team}</strong> players</div><div class="meta">${s.board_buyers||0} FourBag purchasers</div></article>`).join(''):'<div class="card muted">No open seasons yet.</div>'}catch(e){leagueGrid.innerHTML=`<div class="card error">${e.message}</div>`;seasonSelect.innerHTML=''}}
document.getElementById('refresh').addEventListener('click',loadSeasons);document.getElementById('registerForm').addEventListener('submit',async e=>{e.preventDefault();result.textContent='Saving...';const form=new FormData(e.currentTarget),payload=Object.fromEntries(form.entries());payload.board_purchase=form.get('board_purchase')==='1';try{const data=await api('player.register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});result.textContent=`Registered. Player #${data.player_id}.`;e.currentTarget.reset();await loadSeasons()}catch(err){result.textContent=err.message}});loadSeasons();
</script>
</body>
</html>
